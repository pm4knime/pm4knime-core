 ...
 *
 */
public class ManifestWithSerializer {
	...
	public static void saveTo(ManifestEvClassPattern manifest, File internDir, ExecutionMonitor exec) throws IOException, CanceledExecutionException {
		...
		XEventClassifier classifier = manifest.getEvClassifier();
		// not know if it is attribute or normal classifier
		String clfName = classifier.name();
		...
		for(XEventClass ecls : manifest.getMoveLogCost().keySet()) {
			eventNames[idx] = ecls.getId();
		...
		TransClass2PatternMap tpMap = manifest.getTransClass2PatternMap();
		...
		// need to save the event classifier, the name and classifier name 
		for(XEventClass ec : tpMap.getEvClass2Enc().keySet()) {
			ecNames[idx] = ec.getId();
	...
	public static ManifestEvClassPattern loadFrom(File internDir, ExecutionMonitor exec)
        ...
        XEventClassifier classifier = XLogUtil.getEventClassifier(log, clfName);
		...
		XEventClass[] evClassEnc = new XEventClass[eventNames.length];
		Map<XEventClass, Integer> mapEvClass2Cost = new HashMap();
		for(idx = 0; idx <eventNames.length; idx++ ) {
			String name = eventNames[idx];
			XEventClass ecls = XLogUtil.findEventClass(name, eventClasses);
			evClassEnc[idx] = ecls;
		...
		Map<XEventClass, Short> evClass2Enc = new HashMap();
		...
		TransClass2PatternMap transClass2PatternMap = new TransClass2PatternMap();
		...
		ManifestEvClassPattern manifest = new ManifestEvClassPattern(anet.getNet(), anet.getInitialMarking(), anet.getFinalMarkings().toArray(new Marking[0]),
