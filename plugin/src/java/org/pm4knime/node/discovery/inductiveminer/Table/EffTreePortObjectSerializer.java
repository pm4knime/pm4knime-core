...
public class EffTreePortObjectSerializer extends PortObjectSerializer<EfficientTreePortObject> {
	...
	@Override
	public void savePortObject(EfficientTreePortObject portObject, PortObjectZipOutputStream out, ExecutionMonitor exec)
	...
	@Override
	public EfficientTreePortObject loadPortObject(PortObjectZipInputStream in, PortObjectSpec spec, ExecutionMonitor exec)
			throws IOException, CanceledExecutionException {
		...
		EfficientTreePortObject portObj = null;
		try {// here we need to take care about the environment, we need to create a context, because we don't have one
            portObj = new EfficientTreePortObject();
            portObj.loadFromDefault((EffTreePortObjectSpec)spec, in);
        } catch (Exception e) {
