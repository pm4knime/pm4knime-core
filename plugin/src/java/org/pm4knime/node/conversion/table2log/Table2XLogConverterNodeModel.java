...
/**
 * <code>NodeModel</code> for the "CVS2XLogConverter" node. This node is coded
 * without ProM plugin. The reasons are : Input: DataTable Output:
 * XLogPortObject Parameters: a lot ! But in two kinds: 1. to assign the caseId
 * and eventID, timestamp according to the Spec from the cvs 2. to choose the
 * storage format of XLogPortObject
 * 
 * For later use, if we want to convert event log into cvs file, and then store
 * them, what to do then?? By using the prom Plugin, we have the exporter and
 * importer available. And then after this, we get the
 * 
 * @author Kefang Ding
 */
public class Table2XLogConverterNodeModel extends NodeModel {
	...
	/**
	 * they must portOBject to PortObject, else not working. {@inheritDoc}
	 */
	@Override
	protected PortObject[] execute(final PortObject[] inData, final ExecutionContext exec) throws Exception {
		...
		BufferedDataTableSorter sorter = new BufferedDataTableSorter(tData, m_inclList, m_sortOrder, m_missingToEnd);

		sorter.setSortInMemory(m_sortInMemory);
		BufferedDataTable sortedTable = sorter.sort(exec);

		checkCanceled();
		// convert the string to date and sort them according to caseID? So we can read
		// them easier for rows
	...
	protected PortObjectSpec[] configure(final PortObjectSpec[] inSpecs) throws InvalidSettingsException {
		...
		// here check the options from the exlcuded list from traceAttrSet and event
		// Attr set
		...
		if (!tsName.equals(SMTable2XLogConfig.CFG_NO_OPTION)
			...
			throw new InvalidSettingsException(
					"The time stamp doesn't have the required format in LocalDateTime or ZonedDateTime");
		...
		throw new InvalidSettingsException(
				"Make sure the attribute set choice panel includes all" + " the choices in the panel!");
