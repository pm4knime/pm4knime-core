...
public class PM4KNIMEPluginManager implements PluginManager {
	...
	public PM4KNIMEPluginManager(Class<? extends PluginContext> pluginContextType) {
...
//	}
	...
	private String loadClass(ClassLoader loader, URL url, String className, PackageDescriptor pack) {
		...
		try {
			...
			if (pluginClass.isAnnotationPresent(Plugin.class) && isGoodPlugin(pluginClass, methods)) {
				PM4KNIMEPluginDescriptor pl = new PM4KNIMEPluginDescriptor(pluginClass, pluginContextType, pack);
				addPlugin(pl);
				isAnnotated = true;
			}
	...
	private void addPlugin(PM4KNIMEPluginDescriptor pl) {
		PM4KNIMEPluginDescriptor old = (PM4KNIMEPluginDescriptor) plugins.put(pl.getID(), pl);
...
//	}
	...
	private boolean isGoodPlugin(Class<?> type, Method[] methods) {
		try {
			...
			// Check if there is at least one method annotated with
			// PluginVariant
